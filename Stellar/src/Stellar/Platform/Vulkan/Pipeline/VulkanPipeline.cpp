...
namespace Stellar {
	...
	void VulkanPipeline::setUniformBuffer(STLR_Ptr<Buffer> uniformBuffer, uint32_t binding, uint32_t set) {
		// STLR_Ptr<VulkanShader> vulkanShader = STLR_Ptr<VulkanShader>(m_Specification.shader);
		// STLR_Ptr<VulkanUniformBuffer> vulkanUniformBuffer = uniformBuffer.As<VulkanUniformBuffer>();

		// STLR_CORE_ASSERT(m_DescriptorSets.DescriptorSets.size() > set);
		...
		// STLR_CORE_WARN("VulkanPipeline - Updating descriptor set (VulkanPipeline::SetUniformBuffer)");
